...
workflow MAKE_CHUNKS {
    ...
    // Index VCF
    BCFTOOLS_INDEX(ch_reference)

    // Reorder channel
    ch_vcf_csi = ch_reference.join(BCFTOOLS_INDEX.out.csi)

    // Make chunks
    ch_vcf_csi_chr = ch_vcf_csi.map{meta, vcf, csi -> [meta, vcf, csi, meta.id]}
    ...
    BCFTOOLS_NORM(ch_vcf_csi, ch_fasta)
